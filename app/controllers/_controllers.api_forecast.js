...
exports.index = async (req, res) => {
    ...
    try {
        ...
        var sites = await ModelSite.findAll({
            attributes: ['site_id', 'site_name', 'site_group'],
            group: ['site_group'],
            where: {
                site_group: {
                    [Op.not]: null
       ...
       // console.log(sites)
        ...
        res.render('test_forecast/_form.ejs', {
            formats: formats,
            sites: sites,
            countrys: countrys
        });
...
console.log(sitesIdsArray)
    ...
    try {
        ...
        if (format.length != 0 && sites.length != 0 && countries.length != 0) {
            ...
            const siteIds = await ModelSite.findAll({
                attributes: ['site_id'],
                where: {
                    site_group: {
                        [Op.eq]: sites
            ...
            // Create siteIdsArray's variable to handler more later
            for (let l = 0; l < siteIds.length; l++) {
                sitesIdsArray.push(siteIds[l].site_id);
            }
            // select groupe format + format id
        ...
        if (format === "INTERSTITIEL") {
            for (let m = 0; m < sitesIdsArray.length; m++) {
            ...
            return res.render('test_forecast/_data.ejs', {
                table: table
            });
        }
