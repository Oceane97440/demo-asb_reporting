    ...
    try {
        ...
        if (format === "INTERSTITIEL") {
            ...
            const requete = await sequelize.query(
                'SELECT * FROM asb_epilot_campaigns WHERE ((campaign_start_date BETWEEN ? AND ?) OR (campaign_end_date BETWEEN ? AND ?)) AND format_name IN(?) ORDER BY asb_epilot_campaigns.format_name ASC', {
